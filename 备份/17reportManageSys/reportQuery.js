...
var reportQuery = {
	      init : function(){
              reportQuery.appendDiv();
              reportQuery.changeTip();
              reportQuery.loadTree();
              //$("#tree").tree('expand',$("#tree").tree('find',0).target);
              //$("#tree").tree('select',$("#tree").tree('find',20160329113533).target);
	      },
        appendDiv : function(){
             var divS = '<div style="cursor:pointer;display:inline-block;margin-right: 30px;float: right;" onClick="reportQuery.loadTree()">'+
                      ...
                      if(data.length>0){
                               for(var i=0;i<data.length;i++){
                                       ...
                                       if(data[i].type == 0){
                                            ...
                                            array_two = reportQuery.zhuanhuaunTreeData(data[i].children);
                                         };
                  ...
                  //------------------------------------------------------------------------------
                   ...
                   $.ajax({
                                ...
                                success : function(data) {
                                     ...
                                     var initData = reportQuery.zhuanhuaunTreeData(dataArr);
                                     //loadTree
                                     $('#tree').tree({
                                         data:initData,
                                         lines : true,
                                         ...
                                         onDblClick: function(node){
                                                   ...
                                                   if(!type){
                                                      //点击打卡一张报表
                                                          ...
                                                          var url = "publicReport.jsp?fromWhere=reportQuery"
                                                          if(!$('#tabs').tabs('exists',subtitle)){
                                                            $('#tabs').tabs('add',{
                                                              title:subtitle,
                                                              content:reportQuery.createFrame(url),
                                                              closable:true,
                                         ...
                                         onContextMenu : function(e,node){
                                                    ...
                                                    if(!node.attributes.ishasChilds){
                                                            if( node.iconCls == "icon-wenbenfabu"){
                                                                  $('#mmm').menu('show', {
                                                                          left: e.pageX,
                                                                          top: e.pageY
                                                                     });
        ...
        addMyAttention : function(){
                ...
                if (res.resultCode == 0 || res.msg == "success") {alert("添加成功");}else{alert("添加失败!!!");};


            //-----------------------
             var node = $('#tree').tree('find',curr_id);
             if (node){
                    $('#tree').tree('update', {
                      target: node.target,
                      iconCls: 'icon-wenbenguanzhu'
                    });
             }
            //------------------- 
        ...
        createQueryResultTab : function(val){       //--------
                                 ...
                                 title_name = res.data[0].name_;
                                   var url = "reportQueryResult.jsp?fromWhere=reportQuery";
                                         ...
                                         for(var i=0; i<item.length;i++){
                                             var groupNum = i%lie;
                                                ...
                                                if(groupNum == 0){
                                                  ...
                                                  htmlStr += '<td style="border-top:0px;width: 33%;"><a id="inputKuang_'+item[i].id_+'" onmouseover="reportQuery.popoverKuang('+item[i].id_+')" onmouseout="reportQuery.delpopoverKuang('+item[i].id_+')" onClick="reportQuery.selectReportName('+item[i].id_+')" style="color: blue;text-decoration:underline ;cursor: pointer;font-size: 17px;" >'+item[i].name_+'</a></td>'
                                               if(groupNum == lie-1){
                                                    htmlStr += '</tr>';
                                                }; 
                                         };
