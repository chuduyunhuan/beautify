...
var addReport = {
        ...
        initXialaType : function(){
                 $("#xialaType").html("");
                 var dataA = {"ifId":"srpt-enum-compDropDown"};
        ...
        initDataPool : function(){
                 ...
                 //找到当前报表的数据源
                  ...
                  var dataA = {"ifId":"srpt-cfg-menuQueryself",
                               "id":updateReportId
                                };
                  var dataStrA = JSON.stringify(dataA);
                        ...
                        for(var i=0;i<zujianArrayNew.length;i++){
                                ...
                                //-------------------------------------
                                        ...
                                        if (zujianArrayNew[i].type =="checkbox") { 
                                          jj++;
                                          if(i == zujianArrayNew.length-1){htmlStr += '</tr></table>';};   //防止最后一个是特殊组件,这样普通组件会缺失
                                          continue;};
                                        if (zujianArrayNew[i].type =="leftRight") {
                                          jj++;
                                          if(i == zujianArrayNew.length-1){htmlStr += '</tr></table>';};
                                          continue;};
                             ...
                             if (zujianArrayNew[i].type == "text" || zujianArrayNew[i].type == "select") {         //判断是否添加普通组件
                              ...
                              if( ii+1 == (zujianArrayNew.length - jj)){htmlStr += '</tr></table>';}; 
                           }; //判断是否添加普通组件
	      ...
	      addZujian : function(){
                        ...
                        //-----------------------------------------
                        var innerHTML = "";  
                        if (zujianLabel == " " || zujianLabel == "") {alert("请输入组件名称!");}else{
                     ...
                     $("#zujianSelect").find('div').each(function(index, el){
                              ...
                              $(this).find("span").each(function(index,el){
                                     if(index == 0){
                                        ...
                                        if($(this).html().indexOf("时间")>-1){return;};
                                        obj.label = $(this).html();
                                      };                               
                                      if(index == 1){
                                        if($(this).html().indexOf("Time")>-1){return;};
                                        obj.id = $(this).html();
                                      };
	      ...
	      save : function(){
               ...
               if(reportName == "" ||reportName.trim() == ""){alert("名称不能为空!");return;}; 
        ...
        updateReportGetParentData : function(){
                   ...
                   var otherCondition = value[3];                    //数组,包含对象
                    ...
                    if(lianxuOrlisan == "lianxu"){otherConditionArr.unshift(startTime,endTime);};
                    if(lianxuOrlisan == "lisan"){
          ...
          checkUpdateMenueName : function(name){
                       ...
                       var dataC = {"ifId":"srpt-cfg-menuQueryParent",
                        ...
                        for(var i=0;i<result.length;i++){
                         ...
                         //查询该同级的名称
                            var dataB = {"ifId":"srpt-cfg-menuQuery",
                                         "parent_id":parent_id
                                        };
                            var dataStrB = JSON.stringify(dataB);   
