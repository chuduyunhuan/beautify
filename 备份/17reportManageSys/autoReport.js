        ...
        appendDiv : function(){
	                    ...
	                    if(data.length>0){
                             for(var i=0;i<data.length;i++){
                                     ...
                                     if(data[i].type == 1){
                                          ...
                                          object.attributes = {"ishasChilds":false,"parent_id":data[i].parent_id,"url":data[i].url};
                                     };
                                     if(data[i].type == 0){
                                          flag = true;
                                          object.attributes = {"ishasChilds":true,"parent_id":data[i].parent_id,"url":data[i].url};
	      ...
	      loadTree : function(getData){
                     var initData = [{
                                ...
                                "state":"open",
                                //"iconCls":"icon-add",
                                "attributes":{"ishasChilds":true,"url":autoReportParam},
                                "children":getData
                      }]; 
                     $('#tree').tree({
		                         data:initData,
		                         ...
		                         onContextMenu : function(e,node){
                                        if(operate == false || operate == "false"){
                                        }else if (operate == true || operate == "true") {
      		                                    e.preventDefault();
          ...
          addMulu : function(){
                    if (autoReportParam == "") {eastcom.showMsg("danger","链接参数不能为空!");setTimeout('clearMsg()', 3000 );return;};
          ...
          addMulus : function(){
          	        ...
          	        if(name.trim().length == 0){alert("名称不能为空!");return;};
                    ...
                    //添加页面节点---------------------------------------------------
                       ...
                       $('#tree').tree('append', {
                        ...
                        data: [{
                              ...
                              text: name,
                              state:"closed",
                              attributes :{"ishasChilds":true,"parent_id":parent_id,"url":url_url},
                              children:[]
                            }]
                       });
              ...
              //添加页面节点---------------------------------------------------
                 ...
                 $('#tree').tree('append', {
                  ...
                  data: [{
                        ...
                        attributes :{"ishasChilds":false,"parent_id":parent_id,"url":url_url}
                      }]
                 });
                  //$("#tree").tree("collapse",parent.target);
                  $("#tree").tree("expand",parent.target);
          ...
          bindReports : function(){
                       ...
                       if (checkedNodes.length == 0) {eastcom.showMsg("danger","请勾选绑定的报表!");return;}
